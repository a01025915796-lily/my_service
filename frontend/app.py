...
BACKEND_ERROR_MSG = "백엔드 서버(8000번)에 연결할 수 없습니다. BE 터미널이 켜져 있는지 확인하세요."
...
with tab_write:
    st.caption("방문 지역과 만족도를 남기는 탭입니다.")
    ...
    if submitted:
        if not record_name:
            ...
            try:
                ...
                st.error(BACKEND_ERROR_MSG)

with tab_my:
    st.caption("이름으로 내가 남긴 기록을 조회하고 삭제하는 탭입니다.")
...
with tab_all:
    st.caption("전체 통계, 지역별 그래프, 전체 기록과 CSV 내보내기를 모아 둔 탭입니다.")
